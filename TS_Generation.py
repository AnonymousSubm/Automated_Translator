...
def state_normal_representation(states):
    normal = []
    for state in states:
        normal.append(transform_to_normalform(state))
    return normal
...
def action_constraints_analysis(action_constraints, atoms_current_state, atoms_goal_state,domain,constants):
    ...
    for constraint in instantiated_constrains:
        ...
        substitution = predicate_existential_variables_instantiation(atoms_goal_state, a_goal_predicate[0],constants)
        partial_instantiated = rule_partial_instantiation(constraint, substitution)
        fully_instantiated = []
        for rule in partial_instantiated:
            fully_instantiated = fully_instantiated + existential_variable_rule_instantiation(rule, atoms_current_state,constants)
...
def event_analysis( received_messages, event_processing, atoms_current_state, atoms_goal_state, domain, constants):
     event_updates={"beliefs":[],"goals":[],"sent_messages":[]}
     enableness_rule = []
             ...
             for msg in processed_received_first_messages:
                 ...
                 if sub1 != []:
                     ...
                     for rule in partial_instantiated:
                         fully_instantiated = fully_instantiated + existential_variable_rule_instantiation(rule,atoms_current_state,constants)
                     for rule in fully_instantiated:
                         if rule[0] in processed_received_first_messages and rule[1] in processed_received_second_messages:
                             new_rule=[True]
                             i=2
                             while i < len(rule):
                                 new_rule.append(rule[i])
                             ...
                             if enabled != []:
                                 if enabled[0] not in keywords:
                                     ...
                                     if enabled[1] not in atoms_current_state:
                                         event_updates["beliefs"].append(enabled[1])
                                         atoms_current_state.append(enabled[1])
                                 elif enabled[0]=="adopt":
                                     if enabled[1] not in atoms_goal_state and enabled[1] not in atoms_current_state:
                                         event_updates["goals"].append(enabled[1])
                                         atoms_goal_state.append(enabled[1])
                 j=j+1
...
def system_generation(agents, knowledge_base, constraints_of_action_generation,enableness_of_actions, action_specification, sent_message_update, event_processing,domain,linear_mode,constants):
    initial_state = {}
    initial_state_rep = {}
    all_agents_name=[]
    for agent in agents:
    ...
    enabled_sent_messages_dict_rep={}
    ...
    while current_states != []:
        ...
        for item in current_states_copy:
            ...
            for agent in agents:
                ...
                # subgoal_state stores te state generate at each subgoal level (for each agent)
                substate = state[name]
                ...
                if current_goal==desired_goal:
                ...
                elif current_goal < desired_goal:
                    substate_dict.update({name:substate})
                    substate_dict_rep.update({name:transform_mental_states_normal(substate)})
                    ...
                    inserted_beliefs = enabled_event_update['beliefs']
                    inserted_beliefs_rep = state_normal_representation(inserted_beliefs)
                    adopted_goals = enabled_event_update['goals']
                    ...
                    if adopted_goals!=[]:
                        for goal in adopted_goals:
                            if goal not in substate_dict[name][1][0]:
                                substate_dict[name] = (substate_dict[name][0], [substate_dict[name][1][0]+[goal]])
